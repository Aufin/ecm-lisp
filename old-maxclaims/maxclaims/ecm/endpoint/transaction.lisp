...
(defmacro plet* (nil &body body)
  `(let* ((date (ecm/hunchentoot:parameter-or-nil "transaction-date"))
		  (date-paid (ecm/hunchentoot:parameter-or-nil "transaction-date-paid"))
	      ...
	      (interim-id (ecm/hunchentoot:parameter-or-nil "interim-id"))
...
(define-endpoint edit-page
  "ecm/transaction/(\\d+)/edit")

(defun edit-page/get (tid) (edit-inline/get tid :inline nil))
(defun edit-page/post (tid) (edit-inline/post tid :inline nil))
...
(defun edit-inline/get
  ...
  (ecm/request-context:with-request-context ()
    (let ((transaction (ecm/entity/transaction:find-transaction
			transaction-id)))
      (ecm/ui/transaction:edit-transaction-page transaction :inline inline))))
      ...
      (let ((transaction (ecm/entity/transaction:find-transaction
        ...
        (flet ((transaction (&key (error nil) (transaction transaction))
	               (ecm/ui/transaction:edit-transaction-page
		              transaction :inline inline :error error
		                          :date date
		                          :type type
		                          :heading heading
		                          :expense-type expense-type
	        ...
	        (handler-case
	            (progn
				  ;;(break "Got interim? ~A" interim-id)
				  (let ((transaction
