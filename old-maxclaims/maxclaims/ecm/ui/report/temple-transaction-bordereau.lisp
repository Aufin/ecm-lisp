(defpackage :ecm/ui/report/temple-transaction-bordereau
  (:use :cl)
  (:import-from :ecm/ml #:<>)
  (:import-from :ecm/ps #:{} #:|.|)
  ...
  (:import-from :ecm/report/temple-transaction-bordereau
                :temple-transaction-bordereau)
  (:import-from :ecm/ui/spreadsheet  #:<spreadsheet-type-select>))
(in-package  :ecm/ui/report/temple-transaction-bordereau)

(defun <report-form> ()
  (<> '(form :method "POST" 
        :action "/ecm/report/temple-transaction-bordereau"
        :target "report-frame")
...
(defun <temple-transaction-bordereau-page>  ()
  (<> '(ecm/ui/page:page :title "Temple Transaction Bordereau : ECM")
        (<> 'style 
...
.col-centered {
    ...
    margin-right:-4px;
        ...
        (<> '(div :class "row row-centered")
          (<> '(html5:h1) "Temple Transaction Bordereau")
          (<> '(div 
...
(defun temple-transaction-bordereau-post (contract-numbers
                                          start-date
                                          interval)
...
Numbers : ~A~%start:~A ~%interval: ~A"
   ...
   (maxclaims::with-adb 
        (let ((result 
               (temple-transaction-bordereau contract-numbers start-date interval)))
...
(hunchentoot:define-easy-handler
    (ttb-report-handler
     :uri "/ecm/report/temple-transaction-bordereau")
    (contract-numbers
     start-date
     interval
     json
  ...
  (if (string-equal "POST" (hunchentoot:request-method*))
      (with-output-to-string (sexpml:*sexpml-output*)
        (let ((js-array (temple-transaction-bordereau-post 
                         contract-numbers
                         start-date
                         interval)))
          (if json 
              ...
              (if (string= js-array "[]")
                  ...
                  (let* ((ss (ecm/report/temple-transaction-bordereau:temple-transaction-bordereau-spreadsheet js-array))
      ...
      (<temple-transaction-bordereau-page>)))
