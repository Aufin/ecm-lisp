...
(defun gnumeric-spreadsheets (&optional (document (error "must provide document")))
...
(defun create-spreadsheet-from-alist
    (name alist
		 &key
       (format-dollarsign t)
		   (create-header? t)
       (create-header-columns? t)
		   (start-row 0)
		   (start-column 0)
       (format-date "~{~A-~A-~A~}"))
	        ...
	        (<> ("gnm:Cells")
	            (loop :for line in report
		                ...
		                :do (loop :for (name . original-value)  in line
		                          ...
		                          (let* ((value
			                                (cond
				                                ((or (typep original-value 'simple-date:timestamp)
                                             (typep original-value 'simple-date:date))
				                                 (format nil format-date
					                                       (multiple-value-list
...
(defun column-identifier (number)
  (let ((*print-base* 36))
      (if (<= 26 number)
	  (princ-to-string (+ 334 (* (truncate number 26) 10) number))
...
(defun create-spreadsheet 
	 ...
	 (title-max-row 
	  (if (not title) 
         ...
         (sheet (create-spreadsheet-from-alist 
                 ...
                 :create-header-columns? (not use-document-columns)
 ...
 (let* ((cell-rows (mapcar #'cell-row 
	   ...
	   (cell-max-row (apply #'max (or cell-rows (list cell-min-row))))
	   (max-row-cells (remove-if-not (lambda (n) (eql cell-max-row n))
					 (remove-if-not (lambda (e) (typep e 'stp:element))
							(stp:list-children (spreadsheet-cells sheet)))
					 :key #'cell-row)))
      ;; For the Totals: row
   (dolist (c (when calculate-totals
		max-row-cells))
