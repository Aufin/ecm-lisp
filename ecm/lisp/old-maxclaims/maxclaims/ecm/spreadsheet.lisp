(uiop:define-package :ecm/spreadsheet
    (:use :cl :max-ecm/gnumeric/spreadsheet
	  :max-ecm/gnumeric/document)
  ...
  (:reexport :max-ecm/gnumeric/spreadsheet)
  (:reexport :max-ecm/gnumeric/document)
...
(in-package :ecm/spreadsheet)

(format t "Loading spreadsheet no fscking CREATE-*-ALIST here ! lol")

(defun find-cell (spreadsheet row col)
  (setf row (princ-to-string row)
	col (princ-to-string col))
  (stp:find-recursively-if 
   (lambda (a)
     (and (typep a 'stp:element)
	  (equalp (stp:local-name a) "Cell")
	  (equalp (stp:attribute-value a "Row" ) row)
	  (equalp (stp:attribute-value a "Col" ) col)))
   spreadsheet))

(defun cell-value (spreadsheet row col)
  (setf row (princ-to-string row)
	col (princ-to-string col))
  (stp:data (stp:first-child (find-cell spreadsheet row col))))

(defun (setf cell-value) (value spreadsheet row col)
  (setf row (princ-to-string row)
	col (princ-to-string col))
  (let ((cell (find-cell spreadsheet row col)))
    (unless cell (error "No Cell ~A ~A" row col))
    (stp:replace-child cell (stp:first-child cell)
		       (stp:make-text (princ-to-string value)))
    (stp:data (stp:first-child cell))))
  

(defun ssconvert (pathname export-type &rest options)
  "=> /pathname/"
  (let* ((export-pathname-type (ssconvert-type-pathname-type export-type))
	 (import-pathname-type (pathname-type pathname))
	 (new-path (merge-pathnames  
                   (make-pathname
		    :name (concatenate
			   'string (pathname-name pathname)
			   (when (equalp import-pathname-type
					 export-pathname-type)
			     "_export"))
		    :type export-pathname-type)
                   pathname))
        (--export-type= (ssconvert-type-value export-type)))
    (prog1 new-path 
      (uiop:run-program 
       (format nil "ssconvert --export-type=~A ~{~A ~} '~A' '~A'" 
              --export-type= options pathname new-path)))))
           
(defun make-spreadsheet-file
 (spreadsheet &key name (type "html"))
  "=> /pathname/"
  (flet ((write-spreadsheet-file (pathname)
           (alexandria:with-output-to-file  
               (s pathname
                  :if-exists :supersede)
             (stp:serialize 
              spreadsheet (cxml:make-character-stream-sink s)))))    
    (let* ((file-path 
            (typecase spreadsheet
                      ...
                      (path
                       (make-pathname 
                        :directory (string-right-trim
                                    '(#\newline) 
                                    (uiop:run-program 
                                     "mktemp -d -t ecm-spreadsheet-XXXXXX"
                                     :output :string))
                        :name name 
                        :type "xml")))
                 (prog1 path 
                   (write-spreadsheet-file path))))))
           (export-type 
            (find type (ssconvert-export-types)
                  :key (if (find #\: type) 
                            #'ssconvert-type-value
                            #'ssconvert-type-pathname-type)
                  :test #'string=)))
      ...
      (if (string= type (pathname-type file-path))
          file-path
          (ssconvert file-path export-type)))))
...
(define-symbol-macro |man ssconvert|
